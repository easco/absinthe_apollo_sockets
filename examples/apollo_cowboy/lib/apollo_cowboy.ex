defmodule ApolloCowboyExample do
  def start(_mode, _args) do
        ...
        {"/socket/websocket", ApolloSocket.CowboySocketHandler, 
          {
            ApolloSocket.AbsintheMessageHandler,
              ...
              pubsub: ApolloCowboyExample.AbsinthePubSub
          }
        },
      ]}
    ])
    ...
    children = [
        ...
        absinthe_subscriptions(ApolloCowboyExample.AbsinthePubSub),
  ...
  def cowboy_server(port, dispatch) do
    %{     
      type: :supervisor,
      id: :cowboy,
      start: {:cowboy, :start_http, [:http_server, 100, [port: port], [env: [dispatch: dispatch]]]},
    }
  end
end
